...
class Loop(OnnxOpConverter):
    ...
    def _impl_v11(cls, inputs, attr, params):
        ...
        def body_fn(*loop_inputs):
            ...
            with subgraph_scope:
                loop_outputs = subgraph_scope.from_onnx_torch(
...
class If(OnnxOpConverter):
    ...
    def _impl_v1(cls, inputs, attr, params):
        ...
        with then_graph:
            then_expr = then_graph.from_onnx_torch(then_branch, graph_scope.opset, get_output_expr=True)
        with else_graph:
            else_expr = else_graph.from_onnx_torch(else_branch, graph_scope.opset, get_output_expr=True)
...
class GraphProto:
    ...
    def from_onnx_torch(self, graph, opset, freeze_params=False, get_output_expr=False):
        ...
        for init_tensor in graph.initializer:   # paddle这里是false 
...
def from_onnx_torch(model, shape=None, dtype="float32", opset=None, freeze_params=False):
    ...
    with g:
        mod, params = g.from_onnx_torch(graph, opset, freeze_params)
    return mod, params
