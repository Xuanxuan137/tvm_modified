...
class Conv(OnnxOpConverter):
    ...
    def _impl_v1(cls, inputs, attr, params):
        ...
        ####### add by xx
        xx_kernel_shape = infer_shape(inputs[1])
        xx_output_channel = xx_kernel_shape[0]
        attr.update({'channels':xx_output_channel})
        #######
...
class Reshape(OnnxOpConverter):
    ...
    def _impl_v5(cls, inputs, attr, params):
        if get_name(inputs[1]) in params:
            ######### add by xx
            # 如果认为某一层是conv2d的bias，那么把它reshape成3维
            if(("conv2d" in inputs.input_keys[0]) and ("b" in inputs.input_keys[0])):
                out = _op.reshape(inputs[0], (int(infer_shape(inputs[0])[0]), 1, 1))
            #########
            else:
        ...
        ######### add by xx
        # 如果认为某一层是conv2d的bias，那么把它reshape成3维
        elif(("conv2d" in inputs.input_keys[0]) and ("b" in inputs.input_keys[0])):
            out = _op.reshape(inputs[0], (int(infer_shape(inputs[0])[0]), 1, 1))
        ## 但这样带来一个问题，如果不打包所有的conv2d层怎么办？
        #########
        else:
...
class Loop(OnnxOpConverter):
    ...
    def _impl_v11(cls, inputs, attr, params):
        ...
        def body_fn(*loop_inputs):
            ...
            with subgraph_scope:
                loop_outputs = subgraph_scope.from_onnx_paddle(
...
class If(OnnxOpConverter):
    ...
    def _impl_v1(cls, inputs, attr, params):
        ...
        with then_graph:
            then_expr = then_graph.from_onnx_paddle(then_branch, graph_scope.opset, get_output_expr=True)
        with else_graph:
            else_expr = else_graph.from_onnx_paddle(else_branch, graph_scope.opset, get_output_expr=True)
...
class GraphProto:
    ...
    def from_onnx_paddle(self, graph, opset, freeze_params=False, get_output_expr=False):
        ...
        for init_tensor in graph.initializer:   # paddle这里是false   torch这里包含了所有权重节点
            if not init_tensor.name.strip():
                raise ValueError("Tensor's name is required.")
            self._params[init_tensor.name] = self._parse_array(init_tensor)
            self._nodes[init_tensor.name] = new_var(        # 根据输入层新建Var
                init_tensor.name,
                shape=self._params[init_tensor.name].shape,
                dtype=self._params[init_tensor.name].dtype,
            )
        for i in graph.input:   # 这里是True    我的模型这里只有一个输入节点
            # from onnx v0.2, GraphProto.input has type ValueInfoProto,
            #  and the name is 'i.name'
            i_name, i_shape, d_type, i_shape_name = get_info(i)
            if i_name in self._params:      # 我的模型这里都是false，即输入节点不在前面的权重节点里
                ...
                self._num_input += 1    # 输入节点数加一
                if i_name in self._shape:       #这里是True
                    i_shape = self._shape[i_name]       # 记录输入节点shape
                else:
                    if "?" in str(i_shape):
                        warning_msg = (
                ...
                if isinstance(self._dtype, dict):
                    ...
                    dtype = d_type                      # 记录输入节点dtype
                self._nodes[i_name] = new_var(i_name, shape=i_shape, dtype=dtype)
            self._inputs[i_name] = self._nodes[i_name]
        ## 现在self._inputs里是根据输入节点新建的Var
        ## 对于torch，self._params里是根据权重节点新建的Var， 对于paddle，self._params是空的

        # get list of unsupported ops
        convert_map = _get_convert_map(opset)   # 查找不支持的算子
        unsupported_ops = set()
        ...
        for node in graph.node:     # 遍历节点，这里paddle包含了权重节点，torch不包含
            op_name = node.op_type
            attr = self._parse_attr(node.attribute)
            # Create and populate onnx input object.
            inputs = onnx_input()
            for i in node.input:        # 遍历每个节点的所有输入节点
                if i != "":
                    inputs[i] = self._nodes[self._renames.get(i, i)]
            if op_name == "Constant":       # 如果是constant    # 这是新版没有的
                ...
                self._nodes[node.output[0]] = new_var(      # 新建Var
                    node.output[0], shape=list(t_proto.dims), dtype=array.dtype
                )
            else:   # 如果不是constant，即如果是计算节点
                i_name = self._parse_value_proto(node)  # 获取节点名
                node_output = self._fix_outputs(op_name, node.output)   # 获取该节点的输出名
                attr["tvm_custom"] = {}
                attr["tvm_custom"]["name"] = i_name
                attr["tvm_custom"]["num_outputs"] = len(node_output)
                
                op = self._convert_operator(op_name, inputs, attr, opset)   # 新建
...
def from_onnx_paddle(model, shape=None, dtype="float32", opset=None, freeze_params=False):
    ...
    with g:
        mod, params = g.from_onnx_paddle(graph, opset, freeze_params)
    return mod, params
